...
1)Reconnaissance (Разведка): Хакер ищет email-ы сотрудников.
2)Weaponization (Вооружение): Хакер пишет вирус и прячет его в Word-документ (твой макрос).
3)Delivery (Доставка): Хакер запаковывает Word в ISO-образ (который ты ковырял через isoinfo) и кидает на почту.
4)Exploitation (Эксплуатация): Юзер кликает, срабатывает уязвимость или макрос.
5)Installation (Установка): Макрос создает скрытый процесс и дропает файл wsuser.db в папку Notice (ты это нашел!).
6)Command & Control (C2 - Управление): Вирус стучится по URL (твой markettrendingcenter.com), чтобы получить команды от хакера.
7)Actions on Objectives (Действия): Хакер крадет данные или шифрует диск.
